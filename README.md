...
- [Functionalities and Usage](#usage)
    ...
    - [compare-cands](#compare-cands)
    - [plot-cands](#plot-cands)
...
The BLIPSS package contains six executable scripts, which are: <br>
...
### 2. ``compare-cands`` <a name="compare-cands"></a>
Compare periodicity detections across a set of <em>N</em> candidate .csv files generated by ``run-ffa-search``. Within each spectral channel, candidate periods from all files are pooled and clustered (Friends-of-Friends); the highest-S/N candidate in each cluster is kept, tagged with an <em>N</em>-digit binary detection code.

Note that the order of input .csv files matters. When read from left to right, the <em>i</em>-th digit of the <em>N</em>-digit binary code refers to the <em>i</em>-th file in `csv_list`, with a `1` denoting a detection in that file and a `0` a non-detection.
...
---
### 3. ``plot-cands`` <a name="plot-cands"></a>
Produce candidate verification plots for a chosen subset of periodicity candidates from a cross-file comparison .csv file generated by [`compare-cands`](#compare-cands). Each selected candidate (identified by its binary detection code) is re-folded at its detected period across every input data file, and the resulting plot combines periodograms, average pulse profiles, and phase-time diagrams from all files in one figure.
...
Reads plotting parameters from a YAML config file. Key configuration sections:
- `input_data`: `data_dir`, ordered `datafile_list` of data files matching those supplied to `compare-cands`, and optional per-file `beam_labels` annotations
- `candidate_file`: `csvfile`, the cross-file comparison .csv produced by `compare-cands`
- `plotting_parameters`: `codes_plot` list of binary detection codes selected for plotting, output `basename`, list of plot formats (defaults to `['.png']`), output directory (`plot_dir`, defaults to `data_dir`), `periodaxis_log` flag, and `use_latex` flag
- `folding_search_parameters`: FFA folding parameters used to re-fold each candidate (trial period range, `fpmin`, phase-bin range, `ducy_max`, optional running-median detrending via `do_deredden` and `rmed_width`)
- `resource_limits`: maximum data volume (GB) to load into memory per file

Each output plot is named `<plot_dir>/<basename>_ch<channel>_code<code>_period<period>.<format>`.

Execution syntax:
```
plot-cands --config config/plot_cands.yaml 2>&1 | tee <Log file>
```
